...
export default function DonationForm() {
  ...
  useEffect(() => {
    if (inputVal.city) {
      getAllAreasByCity(parseInt(inputVal.city))
        .then((res) => setAreas(res.data))
        .catch((err) => console.log(err));

      //Fetch city name using city id
      getSingleCity(parseInt(inputVal.city))
  ...
  function handleSubmit(e) {
    ...
    toast
      .promise(
        addNewDonation(donorId, donationData),
        {
          pending: "Adding...",
      ...
      .then((res) => {
        uploadFoodImage(res.data.id, image)
      ...
      <Row className="py-3">
        <Col xs={12} md={7}>
          <div className="d-flex justify-content-center align-items-center w-100 mt-3">
            <Form
              className="px-4 shadow rounded w-75"
              onSubmit={(e) => {
            ...
            >
              <h1 className="text-center my-3 text-primary fw-semibold text-uppercase">
                Donate Food
              </h1>
              {/* Donating From */}
              ...
              <Form.Group className="my-2">
                ...
                >
                  ...
                  {foodTypes.map((element) => (
                    <option key={element.id} value={element.foodType}>
                      {element.foodType}
                    </option>
                  ))}
                </Form.Select>
                ...
                <Form.Group>
                  ...
                  <div className="d-flex justify-content-between align-items-center gap-3">
                    <Form.Control
                      ...
                      id="foodItems"
                      className="text-capitalize"
                      placeholder="specify each food items"
                      value={inputVal.foodItems}
                      onChange={handleChange}
                    />
                        ...
                        onClick={() => {
                          inputVal.foodItems &&
                            setFoodItemsList([
                              ...foodItemsList,
                              {
                                item: inputVal.foodItems,
                                quantity: `${inputVal.quantity} ${
                                  inputVal.foodType.toLocaleLowerCase() ===
                                  "diary"
                                    ? "Ltr"
                                    : "Kg"
                                }`,
                  ...
                  {foodItemsList.map((element, index) => (
                    <Row
                      xs={3}
                      key={index}
                      className="d-flex justify-content-between my-2 text-capitalize"
                    >
                      ...
                      <Col className="d-flex justify-content-center align-items-center">
                        <AiFillDelete
                          ...
                          color="red"
                          onClick={() => {
                            const newArr = foodItemsList.filter(

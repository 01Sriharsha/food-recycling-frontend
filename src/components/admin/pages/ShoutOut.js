...
export default function ShoutoutPage() {
  ...
  const validate = () => {
    if (inputVal.foodType.length === 0 || inputVal.foodItem.length === 0
  ...
  const handleSubmit = (event) => {
    ...
    toast.promise(createShoutOut(shoutOutData), {
      ...
      <Container className=''>
        <Form onSubmit={handleSubmit} className="my-3 w-50 p-3 mx-auto">
          ...
          <Form.Group>
            ...
            >
              ...
              {foodTypes.map(foodType => (
                <option key={foodType.id} value={foodType.foodType}>{foodType.foodType}</option>
              ))}
            </Form.Select>
          </Form.Group>
          ...
          <Form.Group className='my-3'>
            ...
            <Form.Control type="text" name="foodItem" placeholder="Enter food item" value={inputVal.foodItem} onChange={handleChange} />
          </Form.Group>

          <Form.Group className='my-3'>
            <Form.Label>How much do you need?</Form.Label>
            <Form.Control type="number" name="quantity" placeholder="Enter quantity" value={inputVal.quantity} onChange={handleChange} />
            <small>Food quantity must be in Kg's</small>
          </Form.Group>
